...
export const ResponsePanel: React.FC<ResponsePanelProps> = ({ response, focused }) => {
    ...
    (input, key) => {
      ...
      if (activeTab === 'body') {
        ...
        if (key.upArrow) {
          ...
          setScrollOffsets({
            ...scrollOffsets,
            body: Math.min(Math.max(0, totalLines - maxVisibleBodyLines), currentScroll + 1),
          });
        } else if (key.pageUp) {
          setScrollOffsets({
            ...scrollOffsets,
            body: Math.max(0, currentScroll - maxVisibleBodyLines),
          });
        } else if (key.pageDown) {
          setScrollOffsets({
            ...scrollOffsets,
            body: Math.min(
              Math.max(0, totalLines - maxVisibleBodyLines),
              currentScroll + maxVisibleBodyLines,
            ),
          });
        } else if (input === 'g') {
          ...
          setScrollOffsets({
            ...scrollOffsets,
            body: Math.max(0, totalLines - maxVisibleBodyLines),
          });
        }
        ...
        if (key.upArrow) {
          ...
          setScrollOffsets({
            ...scrollOffsets,
            info: Math.min(Math.max(0, totalHeaders - maxVisibleBodyLines), currentScroll + 1),
          });
        } else if (key.pageUp) {
          setScrollOffsets({
            ...scrollOffsets,
            info: Math.max(0, currentScroll - maxVisibleBodyLines),
          });
        } else if (key.pageDown) {
          setScrollOffsets({
            ...scrollOffsets,
            info: Math.min(
              Math.max(0, totalHeaders - maxVisibleBodyLines),
              currentScroll + maxVisibleBodyLines,
            ),
          });
        } else if (input === 'g') {
          ...
          setScrollOffsets({
            ...scrollOffsets,
            info: Math.max(0, totalHeaders - maxVisibleBodyLines),
          });
        }
        ...
        if (key.upArrow) {
          ...
          setScrollOffsets({
            ...scrollOffsets,
            cookies: Math.min(Math.max(0, totalCookies - 5), currentScroll + 1),
          });
        } else if (key.pageUp) {
          ...
          setScrollOffsets({
            ...scrollOffsets,
            cookies: Math.min(Math.max(0, totalCookies - 5), currentScroll + 5),
          });
        } else if (input === 'g') {
  ...
  return (
    ...
    >
      <Box justifyContent="space-between">
        <Text>
          <Text bold color={focused ? 'magenta' : 'gray'}>
            📊 Response
          </Text>
            ...
            {activeTab === 'info' ? (
              <Box flexDirection="column">
                ...
                >
                  ...
                  <Box>
                    <Text
                      color={
                        response.time < 200 ? 'green' : response.time < 1000 ? 'yellow' : 'red'
                      }
                    >
                ...
                <Box flexDirection="column" marginTop={1}>
                  ...
                  {(() => {
                    ...
                    const visibleHeaders = headerLines.slice(
                      scrollOffsets.info,
                      scrollOffsets.info + maxVisibleBodyLines,
                    );
                    return (
                      <>
                        ...
                        {headerLines.length > maxVisibleBodyLines && (
                          <Text dimColor italic>
                            [{scrollOffsets.info + 1}-
                            {Math.min(scrollOffsets.info + maxVisibleBodyLines, headerLines.length)}{' '}
                            of {headerLines.length} headers]
                          </Text>
                        )}
                      </>
                    );
              ...
              <Box flexDirection="column" flexGrow={1}>
                ...
                {(() => {
                  ...
                  const visibleLines = lines.slice(
                    scrollOffsets.body,
                    scrollOffsets.body + maxVisibleBodyLines,
                  );
                  ...
                  return (
                    <>
                      ...
                      {totalLines > maxVisibleBodyLines && (
                        <Text dimColor italic>
                          📊 [{scrollOffsets.body + 1}-
                          {Math.min(scrollOffsets.body + maxVisibleBodyLines, totalLines)} of{' '}
                          {totalLines} lines]
                        </Text>
                      )}
                    </>
              ...
              <Box flexDirection="column">
                ...
                {(() => {
                  ...
                  const visibleCookies = cookies.slice(
                    scrollOffsets.cookies,
                    scrollOffsets.cookies + 5,
                  );
                  return (
                    <>
                      {visibleCookies.map((cookie, idx) => (
                        ...
                        >
                          ...
                          {cookie.attributes && <Text dimColor>{cookie.attributes}</Text>}
                        </Box>
                      ))}
                      {cookies.length > 5 && (
                        <Text dimColor italic>
                          [{scrollOffsets.cookies + 1}-
                          {Math.min(scrollOffsets.cookies + 5, cookies.length)} of {cookies.length}{' '}
                          cookies]
                        </Text>
                      )}
                    </>
